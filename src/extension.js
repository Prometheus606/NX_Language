...
//   );
// }
// Ergebnis: Wenn du mit der rechten Maustaste klickst → "Format Document", wird der Code sauber formatiert.
...
//     const descriptions = {
//       puts: "Gibt eine Zeichenkette auf der Konsole aus.",
//       proc: "Definiert eine Funktion.",
...
// Ergebnis: Mit Shift + F12 siehst du alle Vorkommen einer Funktion.
...
//     const symbols = [];
//     const regex = /^proc (\w+)/; // Sucht nach Funktionen
...
//Ergebnis: Die Outline-Ansicht zeigt alle proc-Definitionen an.
...
function activate(context) {
  ...
  //   //words, that will be suggested all the time
              ...
              for (let j = 0; j < splittet_line.length; j++) {
                if (splittet_line[j] === "set") {
                  ...
                  if (
                    ...
                    continue;
                  } //namespace variablen nicht lesen
    ...
    const global_provider = vscode.languages.registerCompletionItemProvider(
      "tcl",
      {
        provideCompletionItems(document, position) {
          const globals = [];
    ...
    const buffer_provider = vscode.languages.registerCompletionItemProvider(
      "tcl",
      {
        provideCompletionItems(document, position) {
          const buffers = [];
